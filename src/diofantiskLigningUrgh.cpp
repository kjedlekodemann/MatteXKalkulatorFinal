...
Linje diofantiskLinje(long long a, long long b, long long c) {
    ...
    long long k = c / base.gcd;

    long long x0 = base.x * k;
    long long y0 = base.y * k;

    long long dx = b / base.gcd;
    long long dy = -a / base.gcd;

    return {x0, y0, dx, dy};
}

std::ostream& operator<<(std::ostream& os, const Linje& l) {
    os << "x0=" << l.x0 << std::endl << "y0=" << l.y0 << std::endl << "dx=" << l.dx << std::endl << "dy=" << l.dy;
    return os;
}